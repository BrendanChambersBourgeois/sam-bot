...
multi_channel = True ## Use if you would like the channel name to be used in the title
default_title = "#manual_ioc" ## If above set to false, provide a default 
...
####
## Get the slack username name for the message
####
...
####
## Get the slack channel name for the message
####
def get_channelname(slack_event):
    channels = slack_client.api_call("conversations.list", types="public_channel,private_channel")['channels']
    if 'channel' in slack_event:
        logger.info(slack_event['channel'])
        for channel in channels:
            if channel['id'] == slack_event['channel']:
                name = channel['name']
                logger.info(name)
                return "#" + name
    else:
            ...
            if event["type"] == "message" and "files" in event:
                for file in event["files"]: 
                    if file["mode"] == "snippet":
                        ...
                        channel = get_channelname(event)
                        title = file["title"]
                        #Work out the title                        
                        if title == "Untitled":
                            #Now check how to display the title
                            if multi_channel:
                                strTitle = channel
                            else:
